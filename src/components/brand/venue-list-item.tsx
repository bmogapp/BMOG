...
export function VenueListItem({ venue, onPress }: { venue: Venue; onPress?: () => void }) {
  return (
    <Pressable
      onPress={onPress}
      ...
      <View className="flex-1 justify-center">
        ...
        <Text className="font-tc-bold text-bmog-fg text-[15px] mt-1">{venue.name}</Text>
        <View className="flex-row items-center gap-1.5 mt-1.5">
          ...
          <Text className="font-mono text-bmog-fg-62 text-[11px]">
            {venue.rating} · {venue.dist}
          </Text>
        </View>
      </View>
      <View className="items-end justify-center">
        <Text className="font-mono-bold text-bmog-ember text-[15px]">${venue.offPeak}</Text>
        <Text className="font-mono text-bmog-fg-38 text-[10px]">/時起</Text>
      </View>
    </Pressable>
  );
}
